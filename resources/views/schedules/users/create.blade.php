...
@section('panel')
    <div class=" flex flex-col w-full h-full items-center gap-1">
        <form id="form" method="post" enctype="multipart/form-data"
            action="{{ route('schedules.user.store', ['type' => $type, 'userId' => $userId]) }}"
            ...
            <x-form.input label="{{ $type == 'child' ? 'Tanggal Jadwal Imuniasi' : 'Tanggal Jadwal Konsultasi' }}"
                name="date" type="date" />
            @if ($type == 'child')
                <x-form.input label="Informasi vaksine" name="note" type="text" />
            @endif
            <div class="flex w-full justify-between">
                <h1>Keterangan</h1>
                <select name="status" id="category" class="rounded border-2 border-[#D690DF] py-2 px-10">
                    <option value="Telah Mendaftar" @if (old('status') == 'Telah Mendaftar') selected @endif>Belum Mendaftar
                    <option value="Belum Mendaftar" @if (old('status') == 'Belum Mendaftar') selected @endif>Telah Mendaftar
