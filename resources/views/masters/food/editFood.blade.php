...
@section('panel')
    <div class=" flex flex-col w-full h-full items-center gap-1">
        <form id="form" method="post" enctype="multipart/form-data"
            action="{{ route('food.update', ['id' => $food->id]) }}"
            ...
            @csrf
            <x-form.input label="Nama Makanan" name="name" type="text" value="{{ $food->name }}" />
            <x-form.input label="Deskripsi" name="description" type="textarea" value="{{ $food->description }}" />
            <x-form.input label="Gambar" name="image" type="file" value="{{ $food->image }}" />
            <div class="flex w-full justify-between">
                <h1>Kategori</h1>
                <select name="category" id="category" class="rounded border-2 border-[#D690DF] py-2 px-10">
                    @foreach ($categories as $category)
                        <option value="{{ $category }}" @if ($food->category == $category) selected @endif>
                            {{ $category }}</option>
                    @endforeach
                </select>
            </div>
